...
| Supabase migrations | **0011/0012/0013 applied to production 2026-09-07** | Applied on `qbvdqkknnuweatptjohi` in order; `agent_cycles.visibility` added and all 3,408 historical cycles reclassified to `public` by operator decision (Anthony, 2026-09-07) so the live dashboard, `/agents` and `/challenge` keep their data. `agent_trades` is empty, so `agent_trades_public` legitimately returns 0. Base-table SELECT grants remain revoked; see section 7. |
