...
async function rpc(method, params = []) {
  ...
  assert(response.ok, 'Base RPC unavailable');
  const payload = await response.json();
  assert(!payload.error && payload.result !== undefined, 'Base RPC rejected read');
...
let session, qr, connecting = false, busy = false, halted = false, index = journal.receipts.length;
...
for (const event of ['session_update', 'session_delete', 'session_expire', 'session_event']) client.on(event, () => halt('La conexión cambió. Detenido; revisa el estado antes de continuar.'));
...
const server = createServer(async (req, res) => {
  ...
  if (req.url === '/state' && req.method === 'GET') return send(200, { message, allowSigning, index, tx: plan.transactions[index], qr: session ? null : qr, canConnect: !session && !connecting && !busy && !halted, canSign: allowSigning && !!session && !busy && !halted && index < 19 });
  if (!['/next', '/connect'].includes(req.url) || req.method !== 'POST') return send(404, {});
