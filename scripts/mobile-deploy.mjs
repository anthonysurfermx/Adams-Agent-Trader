...
const server = createServer(async (req, res) => {
  ...
  if (req.url === '/state' && req.method === 'GET') return send(200, { message, allowSigning, index, tx: plan.transactions[index], qr: session ? null : qr, canSign: allowSigning && !!session && !busy && !halted && index < 19 });
  if (req.url !== '/next' || req.method !== 'POST') return send(404, {});
...
const { uri, approval } = await client.connect({ requiredNamespaces: { eip155: { chains: ['eip155:8453'], methods: ['eth_sendTransaction'], events: ['accountsChanged', 'chainChanged'] } } });
...
void approval().then(async value => {
  try { assertSession(value); }
    ...
    halt('La wallet aprobada no corresponde exclusivamente a la cuenta prevista en Base. Conexión cerrada.');
    qr = null;
    return;
  }
  session = value;
...
}).catch(() => { qr = null; halt('La conexión expiró o fue rechazada. Solicita un QR nuevo.'); });
